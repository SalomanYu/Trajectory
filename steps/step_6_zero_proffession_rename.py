...
def get_average_duration(resumes: list[settings.ResumeGroup]):
...
def rename_zero_professions_by_experience(log:logging, resumes: list[settings.ResumeGroup], default_names:set[settings.DefaultLevelProfession], edwica_db_names:list):
    ...
    for resume in resumes:
        ...
        if job_steps[0].level == 0:
            global_experience = settings.experience_to_months(job_steps[0].general_experience)
            for level_values in settings.DEFAULT_LEVEL_EXPERIENCE:
                ...
                if db_name.strip().lower() in step.experience_post.strip().lower():
                    ...
                    if level_step_is_zero:
                        if previos_current_profession:
                            post_experience = sum(list(map(settings.experience_to_months, [item.experience_duration for item in job_steps[::-1][:step_count+1]])))
                        else:
                            post_experience = settings.experience_to_months(step.experience_post)
    ...
    settings.save_resumes_to_json(log, resumes, filename=settings.STEP_6_JSON_FILE)   
