...
class Way:
    ...
    def remove_repeat_groupes(self) -> None:
        ...
        data = tools.load_resumes_json(log, path=config.JSONFILE.STEP_2.value)
        ...
        tools.save_resumes_to_json(log=log, resumes=data_without_duplicates, filename=config.JSONFILE.STEP_3.value)
    ...
    def join_reset_steps(self, logfile: str = "step_5.log", dataPath: str = config.JSONFILE.STEP_4.value, is_seven_step: bool = False):
    ...
    def detect_profession_experience_time(self, logfile: str = "step_6.log"):
        ...
        log = tools.start_logging(logfile=logfile, folder=self.logging_dir)
        resumes = tools.load_resumes_json(log=log, path=config.JSONFILE.STEP_5.value)
